...
class TestFunctionCaching(unittest.TestCase):
    ...
    def test_sanitize_filename(self): 
        ...
        self.assertGreater(function_caching.FILENAME_MAX_LENGTH, len(function_caching.sanitize_filename(filename)))
    ...
    def test_sanitize_filename_impl(self):
        for c in function_caching.FILENAME_INVALID_CHARACTERS:
            filename = f'foo{c}bar'
            self.assertEqual(f'foo%{ord(c):X}bar', function_caching.sanitize_filename(filename))

        self.assertNotEqual(function_caching.sanitize_filename(r'/'), function_caching.sanitize_filename(r'%2F'))
